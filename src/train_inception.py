# @Fangyuan Yu 
# Modified from HuggingFace Diffusers, Apache 2.0 License
...
This is an inception model derived from {base_model}. The weights were trained on {prompt} using [Inception](https://inception.github.io/).
You can find some example images in the following. \n
{img_str}

Inception for the text encoder was enabled: {train_text_encoder}.
"""
    ...
    tags = ["text-to-image", "inception", "diffusers-training"]
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--concept_learning_ratio",
        type=float,
        default=0.5,
        help="Ratio of training steps to use concept-level caption (instance_prompt) vs instance-level captions",
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
        default=None,
        help="HuggingFace dataset name containing instance captions",
    )
    parser.add_argument(
        "--dataset_text_field",
        type=str,
        default="text",
        help="Field name in the dataset containing the captions",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="inception-model",
...
# TODO: save image from HF dataset locally for dataset loading
class InceptionDataset(Dataset):
    def __init__(
        ...
        text_field="text",  # Add configurable text field
        image_field="image",
    ):
        self.size = size
        ...
        self.concept_prompt = concept_prompt  # Store concept prompt
    ...
    def __getitem__(self, index):
        ...
        # Get instance image and text from dataset
        data_item = self.dataset[index % len(self.dataset)]
        instance_image = data_item[self.image_field]
        ...
        if self.encoder_hidden_states is not None:
            ...
            concept_inputs = tokenize_prompt(
                self.tokenizer,
                self.concept_prompt,
            ...
            # Get instance-specific caption from dataset
            instance_inputs = tokenize_prompt(
                self.tokenizer,
                data_item[self.text_field],  # Use configurable text field
                tokenizer_max_length=self.tokenizer_max_length
            )
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    # Get both concept and instance prompt ids
    concept_ids = [example["concept_prompt_ids"] for example in examples]
    instance_ids = [example["instance_prompt_ids"] for example in examples]
    pixel_values = [example["instance_images"] for example in examples]

    if has_attention_mask:
        concept_attention_mask = [example["concept_attention_mask"] for example in examples]
        instance_attention_mask = [example["instance_attention_mask"] for example in examples]

    # Concat class and instance examples for prior preservation.
    # We do this to avoid doing two forward passes.
    if with_prior_preservation:
        instance_ids += [example["class_prompt_ids"] for example in examples]
        pixel_values += [example["class_images"] for example in examples]
        if has_attention_mask:
            instance_attention_mask += [example["class_attention_mask"] for example in examples]
    ...
    batch = {
        "concept_prompt_ids": concept_ids,
        "instance_prompt_ids": instance_ids,
        "pixel_values": pixel_values,
    }

    if has_attention_mask:
        concept_attention_mask = torch.cat(concept_attention_mask, dim=0)
        instance_attention_mask = torch.cat(instance_attention_mask, dim=0)
        batch["concept_attention_mask"] = concept_attention_mask
        batch["instance_attention_mask"] = instance_attention_mask
...
def main(args):
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers("inception", config=tracker_config)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if use_concept:
                    ...
                    attention_mask = batch["instance_attention_mask"]
    ...
    if accelerator.is_main_process:
        ...
        if args.push_to_hub:
            save_model_card(
                ...
                prompt=args.concept_prompt, # potential issue
